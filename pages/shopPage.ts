...
export class ShopPage {
  ...
  async addProductByName(productName: string) {
    await this.productCard.filter({ hasText: productName }).locator(this.addToCartButton).click()
  }
  ...
  getProductCardByName(name: string) {
    return this.productCard.filter({ hasText: name }).locator(this.productPrice)
  }
}
