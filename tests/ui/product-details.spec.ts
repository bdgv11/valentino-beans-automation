...
test('Verify product name and price consistency between shop list and detail page', async ({
  ...
  const index = 2
  const prodName = (await shopPage.getProductNameByIndex(index).textContent()) || ''
  const prodDescription = (await shopPage.getProductDescriptionByIndex(index).textContent()) || ''
  const prodPrice = (await shopPage.getProductPriceByIndex(index).textContent())?.trim() || ''
  ...
  await expect(detailsPage.getProductDescriptionLocator()).toHaveText(prodDescription)
