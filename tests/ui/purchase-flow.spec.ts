...
test('Successful Purchase Flow (Guest) by product name', async ({ page }) => {
  ...
  await expect(orderDetailsPage.getProductNameLocator(prodName)).toHaveText(prodName)
  await expect(orderDetailsPage.getProductQuantity()).toContainText(prodQuantity)
  await expect(orderDetailsPage.getProductPrice(prodPrice)).toContainText(prodPrice)
  await expect(
    orderDetailsPage.getCustomerName(checkoutData.firstName, checkoutData.lastName),
  ).toBeVisible()

  await expect(orderDetailsPage.getCustomerEmail(checkoutData.email)).toBeVisible()
})
...
test('Validate error msgs when try to checkout', async ({ page }) => {
  ...
  for (const error of expectedErrors) {
    await expect(checkoutPage.getErrorMsgLocator().filter({ hasText: error })).toBeVisible()
  }
