...
test('Verify cart badge and drawer update when adding multiple products', async ({ page }) => {
  // Go to shop page
  ...
  await expect(cartPage.getProductNameLocator().filter({ hasText: prodName1 })).toBeVisible()
  await expect(cartPage.getProductNameLocator().filter({ hasText: prodName2 })).toBeVisible()
})

test('Verify total is correct when add products and increase quantity', async ({ page }) => {
  // Go to shop page
  await headerPage.goToShopProduct()
  ...
  await expect(cartPage.getSubtotalLocator().filter({ hasText: subTotal.toString() })).toBeVisible()
})

test('Verify cart badge is updated when remove multiple products', async ({ page }) => {
  // Go to shop page
  await headerPage.goToShopProduct()
